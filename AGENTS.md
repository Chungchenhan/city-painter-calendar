...
- 修改照片或附件的縮圖／清晰圖切換、WebP、載入／失敗提示、固定尺寸、縮放拖曳、滑動關閉、左右切換或導覽按鈕時，必須同時盤點並更新 ERP 與都市彩繪行事曆，不得只修改單一專案。
- ERP 與行事曆的 `src/components/shared/AttachmentImageViewer.tsx` 必須保持逐位元一致；各專案僅能在外層 wrapper 處理樣式、版面殼層與業務資料。
- 完成前必須在兩個專案執行 `node scripts/verify-attachment-viewer-sync.mjs` 與 `npm run build`，並驗證桌面版、手機版的照片載入、縮放、滑動關閉及左右切換。
- 盤點範圍至少包含 ERP 相簿中心、銷貨附件中心、客戶銷售紀錄，以及行事曆工作附件中心與留言附件；未套用共用檢視器的頁面必須明確說明原因。
